...
export default async function AdminSettingsPage() {
  ...
  // Bitácora, correos enviados y la lista de administradores son cosas que un
  // admin de alcance 'limited' no debe ver ni tocar (la RLS de audit_log y
  // email_log ya lo bloquea; la de profiles permite lectura del propio perfil
  // y de todos si es admin, así que aquí se filtra también en la aplicación
  // para que un admin limitado no vea ni la lista de otros administradores).
  const [{ data: audit }, { data: emails }, { data: adminProfiles }] = context.isFullAdmin
    ? await Promise.all([
        ...
        supabase
          .from('profiles')
          .select('id, full_name, email, admin_scope, created_at')
          .eq('role', 'admin')
    ...
    <>
      ...
      {context.isFullAdmin && (
        <div className="mb-6 grid gap-5 lg:grid-cols-2">
          <Panel
            ...
            description="El acceso limitado no ve bitácora ni correos, ni puede crear otros administradores."
          >
            <AdminUserForm />
          </Panel>

          <Panel title="Administradores" description="Cuentas con acceso al panel administrativo.">
            {(adminProfiles ?? []).length === 0 ? (
              <EmptyState icon="🛡️" title="Sin administradores todavía" />
            ) : (
              <ul className="max-h-[420px] space-y-2.5 overflow-y-auto">
                {(adminProfiles ?? []).map((admin) => (
                  <li
                    key={admin.id}
                    className="flex items-center justify-between gap-3 border-b border-line/60 pb-2.5 text-sm last:border-0"
                  >
      ...
      {context.isFullAdmin && (
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <Panel title="Correos enviados" description="Útil para confirmar si una notificación salió.">
            {(emails ?? []).length === 0 ? (
